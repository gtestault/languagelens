...
func createMultipartForm(f io.Reader, fileName string, splitBy string, metadata *FileMetadata) (*bytes.Buffer, contentType, error) {
	...
	if splitBy == "" {
		err = formWriter.WriteField("split_by", "passage")
		if err != nil {
			return nil, "", errors.Wrap(err, errLabel)
		}
		err = formWriter.WriteField("split_respect_sentence_boundary", "false")
		if err != nil {
			return nil, "", errors.Wrap(err, errLabel)
		}
		err = formWriter.WriteField("split_length", "100000")
		if err != nil {
			return nil, "", errors.Wrap(err, errLabel)
		}
	}
...
func (p *Proxy) HaystackFileUpload(buff *bytes.Buffer, ct contentType) error {
	...
	proxyReq, err := http.NewRequest(http.MethodPost, p.HaystackAddress+HAYSTACK_API_UPLOAD_PATH, bytes.NewReader(buff.Bytes()))
	if err != nil {
...
func (p *Proxy) proxyQueryToHaystack(w http.ResponseWriter, req *http.Request) error {
	...
	proxyReq, err := http.NewRequest(http.MethodPost, p.HaystackAddress+HAYSTACK_API_QUERY_PATH, bytes.NewReader(buff.Bytes()))
	if err != nil {
		return err
	}
